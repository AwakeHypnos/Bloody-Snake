...
class Game {
    ...
    nextRoom() {
        if (this.roomsCompleted >= ROOMS_TO_WIN) {
            this.victory();
            return;
        }
        ...
        setTimeout(() => {
            ...
            switch(this.roomType) {
                case ROOM_TYPES.SNAKE:
                    this.currentRoom = new SnakeRoom(this);
                    document.getElementById('room-type').textContent = '🐍 贪吃蛇房间';
                    break;
                case ROOM_TYPES.SHOOTER:
                    this.currentRoom = new ShooterRoom(this);
                    document.getElementById('room-type').textContent = '🔫 横版射击房间';
                    break;
    ...
    showTransition() {
        ...
        document.getElementById('transition-text').textContent = texts[Math.floor(Math.random() * texts.length)];
    ...
    updateRoomInfo() {
        document.getElementById('room-count').textContent = `房间: ${this.roomsCompleted}/${ROOMS_TO_WIN}`;
    }
...
class SnakeRoom {
    constructor(game) {
        ...
        this.gridWidth = Math.floor(CANVAS_WIDTH / this.gridSize);
        this.gridHeight = Math.floor(CANVAS_HEIGHT / this.gridSize);
    ...
    render(ctx) {
        ...
        const showSnake = this.game.invincibleTime <= 0 || Math.floor(this.flashTimer / (this.flashInterval / 2)) % 2 === 0;
    ...
    renderHUD(ctx) {
        ...
        ctx.fillText(`小苹果: ${this.applesEaten}/${this.applesNeeded}`, CANVAS_WIDTH / 2 - 60, 30);
        
        if (this.roomAppleSpawned && !this.doorsOpen) {
            ctx.fillStyle = '#ffd700';
            ctx.fillText('收集金苹果离开房间！', CANVAS_WIDTH / 2 - 80, 55);
        }
    }
}
...
class ShooterRoom {
    ...
    update(deltaTime) {
        ...
        if (this.backgroundOffset > CANVAS_WIDTH) {
            this.backgroundOffset = 0;
        }
    ...
    handleInput() {
        ...
        if (this.game.keys['ArrowDown'] && this.player.y < CANVAS_HEIGHT - this.player.height - 10) {
            this.player.y += this.player.speed;
        }
    ...
    updateBullets(deltaTime) {
        this.bullets = this.bullets.filter(bullet => {
            ...
            return bullet.x < CANVAS_WIDTH + 50;
        });
    ...
    updateBoss(deltaTime) {
        ...
        if (this.boss.y <= 50 || this.boss.y >= CANVAS_HEIGHT - this.boss.height - 50) {
    ...
    checkAppleCollection() {
        if (this.checkCollision(this.player, this.apple)) {
    ...
    checkDoorCollision() {
        if (this.checkCollision(this.player, this.exitDoor)) {
            this.game.roomComplete();
        }
    }
    ...
    renderHUD(ctx) {
        if (this.boss) {
            ctx.fillStyle = '#fff';
            ctx.font = '16px Courier New';
            ctx.fillText('BOSS 战斗中！', CANVAS_WIDTH / 2 - 50, 30);
        } else if (!this.bossDefeated) {
            ctx.fillStyle = '#fff';
            ctx.font = '16px Courier New';
            ctx.fillText(`波次: ${this.wavesSpawned}/${this.totalWaves}`, CANVAS_WIDTH / 2 - 40, 30);
        } else if (this.apple) {
            ctx.fillStyle = '#ffd700';
            ctx.font = '16px Courier New';
            ctx.fillText('收集金苹果！', CANVAS_WIDTH / 2 - 50, 30);
        } else if (this.exitDoor) {
            ctx.fillStyle = '#06d6a0';
            ctx.font = '16px Courier New';
            ctx.fillText('向右进入出口！', CANVAS_WIDTH / 2 - 60, 30);
        }
    }
}
...
class SpaceshipRoom {
    ...
    update(deltaTime) {
        ...
        if (!this.bossDefeated) {
            if (!this.boss) {
                ...
                if (this.wavesSpawned >= this.totalWaves && this.enemies.length === 0) {
                    this.spawnBoss();
                    this.addComicBubble('BOSS出现！', CANVAS_WIDTH / 2 - 50, CANVAS_HEIGHT / 2);
                }
            } else {
                this.updateBoss(deltaTime);
            }
    ...
    handleInput() {
        ...
        if (this.game.keys['ArrowRight'] && this.player.x < CANVAS_WIDTH - this.player.width - 10) {
            this.player.x += this.player.speed;
        }
    }
    ...
    updateEnemies(deltaTime) {
        this.enemies = this.enemies.filter(enemy => {
            ...
            return enemy.y < CANVAS_HEIGHT + 100 && enemy.health > 0;
        });
    }
    ...
    updateBoss(deltaTime) {
        ...
        if (this.boss.x <= 30 || this.boss.x >= CANVAS_WIDTH - this.boss.width - 30) {
            this.boss.moveDirection *= -1;
        }
        ...
        this.bullets.forEach((bullet, bulletIndex) => {
            ...
            if (this.boss && this.checkCollision(bullet, this.boss)) {
                ...
                if (this.boss.health <= 0) {
                    this.bossDefeated = true;
                    this.boss = null;
                    this.addComicBubble('KABOOM!', CANVAS_WIDTH / 2 - 60, 100);
                    this.spawnApple();
                }
            }
        });
